...
(function($) {
	...
	var Suggest = function(el, key, options) {
		...
		this.$dropdown = $('<div />', {
					class : 'dropdown suggest suggest_'+this.options.primary_key,
					html : $('<ul />', {
								class : 'drop-menu',
								role : 'menu'
							}),
	...
	Suggest.prototype = {
			...
			setTimeout(function() {
				...
				for (var i = currentPos; i >= 0; i--) {
					...
					if (subChar === _self.key
						...
						if(_self.query != ''){
							searchMembesByKey(_self.query,_self.options.token,_self.options.comp_id,_self.options.plat,function(list){
								var temp = [];
								for(var i=0;i<list.length;i++){
									var mem = {
										name:list[i].nick_name,
										value:list[i].nick_name,
										text:list[i].nick_name,
										uuid:list[i].uuid
									}
									temp.push(mem);
								}
		...
		__build : function(qData) {
			...
			$(this.$element.get(0).edit.doc.body).unbind("keydown").on('keydown',
					function(e) {
						...
						if (that.isShown) {
							switch (e.keyCode) {
								...
								case 40 : // arrow down
									...
									var list = $('.suggest_'+that.options.primary_key+' .drop-menu');
									var curNum = list.find('li:first').index();
							        if(list.find('.active').length > 0){
							            curNum = list.find('.active').index()+1;
							        }
							        ...
							        var obj = list.find('li').eq(dirNum-1);
							        var height = list.height();
							        var position = dirNum*(obj.height()+1);
							        var scrollTop = list.scrollTop();
							        if(height + scrollTop <= position){
							            var num = position - scrollTop - height;
							            list.scrollTop(scrollTop + num);
							        }
								...
								case 38 : // arrow up
							        ...
							        var obj = list.find('li').eq(dirNum-1);
							        var height = list.height();
							        var position = (dirNum-1)*(obj.height()+1);
							        var scrollTop = list.scrollTop();
						...
						var content = el.html();
						var members = suggestGetAtMembers(content);
						that.options.atMembers(members);
					});
		...
		__select : function(index) {
			...
			el.insertHtml('<span class="at_person at_person_'+item.uuid+'" data-uuid="'+item.uuid+'">'+this.key+item.value+'</span><span 			id="input-textarea-caret-position-mirror-span">.</span>');
			...
			var content = el.html();
			var members = suggestGetAtMembers(content);
			this.options.atMembers(members);
...
function searchMembesByKey(key,token,comp_id,plat,cb){
	if(plat == 'web'){
		var url = config_server.formal.app01+'/ftask/api/user/search/all.json?token='+token+'&key='+key
		$.ajax({
			type: 'GET',
			url: url,
			data: '',
...
function suggestGetAtMembers(content){
	var members = [];
    $('body').append('<div id="at_suggest_box" style="display:none;"></div>');
    $('#at_suggest_box').html(content);
    $('#at_suggest_box .at_person').each(function(i){
        var $this = $(this);
        var classBox = $this.attr('class');
        var uuid = classBox.replace('at_person at_person_','');
        members.push(uuid);
    });
    var result = members.unique();
    $('body #at_suggest_box').remove();
    return result;
}
