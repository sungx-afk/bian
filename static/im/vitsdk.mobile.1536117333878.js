...
VitSDK.prototype.init = function (strHost, userId, userName, token,  ntfyCallback, statusCallback) {
    ...
    if(window.location.protocol == 'https:'){
        this._wsAddress = "wss://" + strHost.substr(0,strHost.length-1)+'2';
    }

    this._mapReq = new HashMap();
    var protoBuf = dcodeIO.ProtoBuf;

    this._protoMsg = protoBuf.loadProtoFile("https://static-app01.yugusoft.com/static/im/protobuf/vitmsg.v2.proto").build();
...
VitSDK.prototype.sendOnline = function () {
    ...
    req.set_device_type(this._protoMsg.chips_c.DeviceType.DEV_IPAD);//DEV_WX DEV_IPAD DEV_PC
...
VitSDK.prototype.changeStatus = function(status){
    var req = new this._protoMsg.chips_c.ChangeUserStatusReq();
    req.set_id(this.getSequence())
    req.set_status(status)
    this._status = status
    this.sendMsg(req, this._protoMsg.chips_c.RequestType.EChangeUserStatusReq);
}
