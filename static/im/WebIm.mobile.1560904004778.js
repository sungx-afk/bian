...
function WebIm() {
  ...
  var that = this;
  this.closeNetWork = function(){
    that.vitSdk.clear();
  }


  this.changeOnlineStatus = function(status){
    that.vitSdk.changeStatus(status);
  }
...
ContactManager.prototype.syncContact = function (callback) {
  ...
  $.ajax({
    url:webIm.msgUrl+"comm/contact_v3",
    // async:true,
    type:"POST",
    data:data,
    success:function(result) {
...
ChatMessage.prototype.createChatMessage = function (val, chatReq) {
  ...
  else if(this.messageType == 10){//视频消息
      this.messageContent = JSON.parse(this.messageContent);
  }
  else if(this.messageType == 10002){//特殊显示消息，撤回，好友验证
    let t_msg = JSON.parse(this.messageContent);
    if(t_msg.prompt){
      this.messageContent = JSON.parse(this.messageContent);
    }else{
...
WebIm.prototype.initSyncContact = function () {
  ...
  webIm.contactMgr.syncContact(function (result) {
    ...
    if(webIm.initSync.reqNum<=0)
      ...
      console.log("else")
    webIm.initSync.contactData = result;
    console.log(webIm.initSync.contactData)
