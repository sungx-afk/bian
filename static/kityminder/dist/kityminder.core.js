        ...
        kity.extendClass(Minder, {
            ...
            applyLayoutResult: function(root, duration, callback) {
                ...
                function apply(node, pMatrix) {
                    ...
                    // matrix.m.e = matrix.m.e/1.6
                    // matrix.m.f = matrix.m.f/1.6
                    // 如果当前有动画，停止动画
    ...
    value: function(require, exports, module) {
        ...
        Layout.register("mind", kity.createClass({
            ...
            doLayout: function(node, children) {
                ...
                var left = [];
                children.forEach(function(child) {
                    if (child.getIndex() < half) right.push(child); else left.unshift(child);
                });
        ...
        var TreeDragger = kity.createClass("TreeDragger", {
            ...
            dragEnd: function() {
                ...
                if (this._dropSucceedTarget) {
                    ...
                    var opt = this._minder._options || {};
                    if (opt.checkNodeMove) {
                        //检测是否可以拖拽
                        var check = opt.checkNodeMove(this._dragSources, this._dropSucceedTarget);
                        if (check) {
                    ...
                    var opt = this._minder._options || {};
                    if (opt.checkNodeOrder) {
                        //检测是否可以拖拽排序
                        var check = opt.checkNodeOrder(this._dragSources, this._orderSucceedHint);
                        if (check) {
        ...
        Module.register("ProgressModule", function() {
            ...
            var ProgressIcon = kity.createClass("ProgressIcon", {
                ...
                create: function() {
                    ...
                    bg = new kity.Circle(7).fill(BG_COLOR);
                    pie = new kity.Pie(7, 0).fill(PIE_COLOR);
                    shadow = new kity.Path().setPathData(SHADOW_PATH).setTranslate(-8, -8).fill(SHADOW_COLOR);
                    frame = new kity.Path().setTranslate(-8, -8).setPathData(FRAME_PATH).setScale(.8).fill(FRAME_GRAD);
                    check = new kity.Path().setTranslate(-8, -8).setPathData(CHECK_PATH).setScale(.8).fill(CHECK_COLOR);
        ...
        var ViewDragger = kity.createClass("ViewDragger", {
            ...
            _bind: function() {
                ...
                this._minder.on("normal.mousedown normal.touchstart " + "inputready.mousedown inputready.touchstart " + "readonly.mousedown readonly.touchstart", function(e) {
                    ...
                    if (e.getTargetNode() == this.getRoot() || !e.getTargetNode()) {
...
_p[69] = {
    value: function(require, exports, module) {
        ...
        template.register("filetree", {
            getLayout: function(node) {
                var opt = this.getMinder()._options || {};
                var lv = opt.filetreeLv == null ? 1 : opt.filetreeLv;
                console.log(node.getLevel(), lv);
                if (node.getData("layout")) return node.getData("layout");
                if (node.isRoot() || node.getLevel() < lv) return "bottom";
...
_p[76] = {
    value: function(require, exports, module) {
        ...
        [ "fishbone", "fishbone-compat" ].forEach(function(name) {
            var compat = name == "fishbone-compat";
            /* jscs:disable maximumLineLength */
            theme.register(name, {
                background: "#fbfbfb",
                "root-color": "white",
                ...
                "root-stroke": "#73cdfb",
                "root-font-size": 16,
                "root-padding": compat ? [ 6, 12 ] : [ 6, 12 ],
                //12, 24]
                "root-margin": compat ? 10 : 10,
                //[30, 100]
                "root-radius": 5,
                "root-space": 10,
                "main-color": "#333333",
                ...
                "sub-stroke": "none",
                "sub-font-size": 12,
                "sub-padding": compat ? [ 3, 5 ] : [ 3, 5 ],
                //[5, 10]
                "sub-margin": compat ? [ 4, 8 ] : [ 4, 8 ],
                //[15, 20]
                "sub-radius": 5,
                "sub-space": 5,
                "connect-color": "#bbb",
                ...
                "marquee-background": hsl(204, 100, 80).set("a", .1),
                "marquee-stroke": hsl(204, 37, 60),
                "drop-hint-color": hsl(204, 26, 35),
                "drop-hint-width": 5,
                "order-hint-area-color": hsl(204, 100, 30).set("a", .5),
