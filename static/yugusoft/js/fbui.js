...
/*供客户端调用，获取程序所有可支持的api_name*/
function getClientData(data){
    var data = getData(data);
    window.allApiName = data.allApiName;
}
/*供客户端调用 本身没有这个方法  目前是客户端有调用  调用报错因此先加上方法*/
...
function changeCronToString(cron,lifecycle){//cron表达式转换为描述文字
    ...
    if((box[5] == '2-6' && box[3] == '?' || box[5] == '?' && box[3] == '*') && box[4] == '*'){
        ...
        function changeWeeks(week){
            var r_week = "";
            switch(week){
                case '1':r_week = '日';break;
                case '2':r_week = '一';break;
                case '3':r_week = '二';break;
                case '4':r_week = '三';break;
                case '5':r_week = '四';break;
                case '6':r_week = '五';break;
                case '7':r_week = '六';break;
            }
            return r_week;
        }
        ...
        for(var i=0;i<week.length;i++){
            result = result + changeWeeks(week[i])
            if(i<week.length-1){
                result = result + ',';
            }
        }
